...
<body>
    ...
    <div class="container py-5">
        ...
        <div class="text-center">
            ...
            <form id="urlForm" class="mb-5">
                ...
                <button type="submit" class="btn btn-primary">Fetch Airdrop Data and Predict</button>
            </form>
    ...
    .then(data => {
        if (data.error) {
            ...
            // Run the prediction immediately after fetching the data
            performPrediction();
        }
    })
    .catch(error => {
...
function performPrediction() {
    // Check if the airdrop URL is still valid before predicting
    const airdropUrl = document.getElementById('airdrop_url').value;
    const airdropUrlPattern = /^https:\/\/airdrops\.io\/[a-z0-9-]+\/?$/;
    ...
    if (!airdropUrlPattern.test(airdropUrl) || bannedUrls.some(bannedUrl => airdropUrl.startsWith(bannedUrl))) {
        alert("Invalid URL. Please enter a valid airdrop URL.");
        return;
    }
...
//document.getElementById('urlForm').querySelector('button[type="submit"]').addEventListener('click', performPrediction);
